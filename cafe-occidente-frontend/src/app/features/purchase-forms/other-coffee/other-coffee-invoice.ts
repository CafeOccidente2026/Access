...
/** Formato colombiano con 2 decimales. */
function money(value: number): string {
  return value.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

/** Formato colombiano sin decimales (precios por unidad). */
...
/**
 * Documento Soporte de compra de Cafés Otros - mismo layout de campos que el reporte real
 * `FacturaEsp` (docs/legacy-vba-export/eltambo/reports/FacturaEsp.txt), verificado control por
 * control contra el mismo reporte de Café Seco: son el mismo set exacto de campos (incluidos
 * Pr_AlmDefec/"Precio Pasilla $"/"Kilos Netos Pergamino", que el reporte real de Access mantiene
 * literalmente aunque el módulo no sea Pergamino Seco - no se encontró ningún campo adicional en
 * FacturaEsp que no esté ya en el documento de Seco). Única diferencia deliberada respecto al VBA:
 * el título no dice "pergamino seco" (el de Access lo dice igual para los dos módulos, aunque para
 * Cafés Otros no es correcto) - decisión de UX sin precedente claro en el VBA, ver resumen del
 * módulo.
 */
export function buildOtherCoffeeInvoiceDocDefinition(
  ...
  const title = `Documento Soporte de compra de café ${purchase.specialType}`;
  ...
  const mainTable: Content = {
    table: {
      ...
      body: [
        ...
        [
          ...dataRow('Precio Carga $:', wholePeso(purchase.basePriceLoad)),
