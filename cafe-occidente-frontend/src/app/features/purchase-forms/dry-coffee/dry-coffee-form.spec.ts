...
import { DryCoffeeFormComponent } from './dry-coffee-form';
...
/**
 * Cubre el gate REQUIRED/hasRequiredCascadeInputs de buildRequest() (dry-coffee-form.ts) - el mismo
 * mecanismo donde vivia el bug del celular bloqueado (ver lookupGrower): un caficultor migrado con
 * telefono vacio dejaba `cellphone` en REQUIRED pero bloqueado y sin valor, y la cascada de calculo
 * (preview()) nunca llegaba a dispararse.
 */
describe('DryCoffeeFormComponent - gate REQUIRED de la cascada', () => {
  const content: PurchaseFormContent & { messages: Record<string, string> } = {
    windowTitle: 'Compras Café Seco',
    theme: 'sky',
    topFields: [],
  ...
  const growerWithoutPhone: Grower = { ...growerWithPhone, idNumber: '27197019', address: 'EL CIDRAL', phone: '' };
  ...
  let component: DryCoffeeFormComponent;

  function setup(grower: Grower) {
    previewSpy = vi.fn(() => of(calculation));
    const fakeDryCoffeeService = {
    ...
    const fakeGrowerService = {
      findByIdNumber: vi.fn(() => of(grower)),
      findProgram: () => of(null),
    };
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        { provide: DryCoffeePurchaseService, useValue: fakeDryCoffeeService },
        { provide: GrowerService, useValue: fakeGrowerService },
      ],
    });
    component = TestBed.createComponent(DryCoffeeFormComponent).componentInstance;
  }
  ...
  it('never calls preview() while a REQUIRED field is still missing', () => {
    ...
    fillCommon();
    fillWeightsAndBags();
    // Sin 'tare' todavia, buildRequest() debe devolver null y preview() no debe llamarse.
  ...
  it('REGRESIÓN: a grower with a blank migrated phone does not get cellphone permanently locked', () => {
    ...
    // Antes del fix: se bloqueaba igual con valor vacio, y la cascada quedaba rota para siempre.
    expect((component as any).locked.has('cellphone')).toBe(false);
    expect(component.model['cellphone']).toBe('');

    // El cajero completa el dato a mano; el gate lo acepta como cualquier otro campo REQUIRED.
    ...
    expect(component.model['cellphone']).toBe('3001112233');
  });

  it('locks address/cellphone/idType normally when the migrated grower data is complete', () => {
    setup(growerWithPhone);
    component.model['fund'] = 'RP';
    component.onFieldCommitted('fund');
    component.model['idPart1'] = growerWithPhone.idNumber;
    ...
    expect(component.model['cellphone']).toBe(growerWithPhone.phone);
  });

  it('clears calc() and surfaces saveError when preview() fails (invalid input mid-cascade)', () => {
    setup(growerWithPhone);
    previewSpy.mockReturnValue(throwError(() => new Error('400')));
    fillCommon();
    fillWeightsAndBags();
    component.model['tare'] = '50';
    component.onFieldCommitted('tare');
  ...
  it('blocks the whole form when the grower is deceased, without touching cellphone/address', () => {
    ...
    expect(component.model['cellphone']).toBeUndefined();
  });
});
